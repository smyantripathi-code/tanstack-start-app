    ...
    <main>
      ...
      <section style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', position: 'relative', overflow: 'hidden', padding: '0 1.5rem' }}>
        ...
        <div style={{
          ...
          backgroundImage: 'linear-gradient(rgba(255,0,106,0.03) 1px, transparent 1px), linear-gradient(90deg, rgba(255,0,106,0.03) 1px, transparent 1px)',
          backgroundSize: '60px 60px',
        }} />
        <div style={{ position: 'absolute', top: '20%', left: '-10%', width: '600px', height: '600px', borderRadius: '50%', background: 'radial-gradient(circle, rgba(255,0,106,0.1) 0%, transparent 70%)', zIndex: 1, pointerEvents: 'none' }} />
        ...
        <div style={{ position: 'relative', zIndex: 2, maxWidth: '1280px', margin: '0 auto', width: '100%', paddingTop: '7rem' }}>
          <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: '0.8rem', color: 'var(--fire)', letterSpacing: '0.15em', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
            ...
            FIRST TECH CHALLENGE · LAKE OSWEGO, OREGON · 2026-2027 SEASON
          </div>
          ...
          <h1 style={{ fontFamily: "'Orbitron', sans-serif", fontSize: 'clamp(3rem, 8vw, 7rem)', fontWeight: 900, lineHeight: 0.9, marginBottom: '1.5rem', letterSpacing: '-0.02em' }}>
            ...
            <span style={{ color: 'var(--fire)', display: 'block', textShadow: '0 0 60px rgba(255,0,106,0.5)' }}>FIRE</span>
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: 'clamp(1rem, 2vw, 1.15rem)', maxWidth: '560px', lineHeight: 1.75, marginBottom: '2.5rem' }}>
            A competitive FTC robotics team from Lake Oswego, Oregon - 16 middle and high school students building robots, writing code, and chasing a premier event berth in our 2026-2027 season.
          </p>
          ...
          <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', marginBottom: '4rem' }}>
            <Link to="/robotics" style={{
              ...
              onMouseEnter={e => (e.currentTarget.style.boxShadow = '0 0 40px rgba(255,0,106,0.7)')}
              onMouseLeave={e => (e.currentTarget.style.boxShadow = '0 0 24px rgba(255,0,106,0.4)')}>
              Our Robot
            </Link>
            <Link to="/team" style={{
            ...
            <Link to="/history" style={{
              background: 'rgba(255,0,106,0.08)', color: 'var(--pink-soft)', textDecoration: 'none',
              padding: '0.85rem 2rem', borderRadius: '0.4rem', fontWeight: 600, fontSize: '0.9rem',
              border: '1px solid rgba(255,0,106,0.2)', transition: 'all 0.2s',
            }}
              onMouseEnter={e => { e.currentTarget.style.background = 'rgba(255,0,106,0.15)' }}
              onMouseLeave={e => { e.currentTarget.style.background = 'rgba(255,0,106,0.08)' }}>
              Our History
            </Link>
          </div>
      ...
      <section style={{ padding: '5rem 1.5rem', background: 'var(--dark-mid)' }}>
        <div style={{ maxWidth: '1280px', margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '3rem', alignItems: 'start' }}>
          <div>
            ...
            <p style={{ color: 'var(--text-muted)', lineHeight: 1.9, marginBottom: '1rem', fontSize: '0.95rem' }}>
              Wire Fire #23260 is a third-year FTC team from Lake Oswego, Oregon. We are 16 middle and high school students learning to excel in engineering, programming, and design, but also in outreach, accounting, and running a real organization.
            </p>
            ...
            <div style={{ marginTop: '1.75rem', display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
              {['STEM-Focused', 'Self-Funded', 'Community-Driven', '501(c)(3)'].map(tag => (
                <span key={tag} style={{
                  background: 'rgba(255,0,106,0.08)', color: 'var(--fire)',
                  border: '1px solid rgba(255,0,106,0.2)', borderRadius: '0.3rem',
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            {highlights.map(h => (
              <div key={h.title} style={{ borderLeft: '2px solid rgba(255,0,106,0.35)', paddingLeft: '1.25rem' }}>
                <div style={{ fontWeight: 700, color: 'var(--white)', fontSize: '0.88rem', marginBottom: '0.35rem', letterSpacing: '0.04em' }}>{h.title}</div>
                <div style={{ color: 'var(--text-muted)', fontSize: '0.85rem', lineHeight: 1.7 }}>{h.desc}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* 2026-2027 SEASON GOALS */}
      <section style={{ padding: '5rem 1.5rem', background: 'var(--dark)', borderTop: '1px solid var(--dark-border)' }}>
        <div style={{ maxWidth: '1280px', margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '3rem', alignItems: 'start' }}>
          <div>
            <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: '0.7rem', color: 'var(--fire)', letterSpacing: '0.2em', marginBottom: '0.5rem' }}>// 2026-2027 Season</div>
            <h2 style={{ fontFamily: "'Orbitron', sans-serif", fontSize: 'clamp(1.5rem, 3vw, 2rem)', fontWeight: 700, color: 'var(--white)', marginBottom: '1rem' }}>Season Goals</h2>
            <p style={{ color: 'var(--text-muted)', fontSize: '0.93rem', lineHeight: 1.9 }}>
              Our 2026-2027 season is our most ambitious yet. After earning 2nd Place Inspire at League Tournament and a Reach Award at State, we are building on that momentum with four concrete targets for the coming year.
            </p>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            {goals2026.map((g, i) => (
              <div key={i} style={{ display: 'flex', gap: '1.25rem', alignItems: 'flex-start' }}>
                <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: '0.65rem', color: 'var(--fire)', letterSpacing: '0.15em', flexShrink: 0, marginTop: '0.25rem', minWidth: '3.5rem' }}>GOAL {String(i + 1).padStart(2, '0')}</div>
                <div>
                  <div style={{ fontWeight: 700, color: 'var(--white)', fontSize: '0.9rem', marginBottom: '0.25rem' }}>{g.label}</div>
                  <div style={{ color: 'var(--text-muted)', fontSize: '0.83rem', lineHeight: 1.65 }}>{g.desc}</div>
                </div>
              </div>
            ))}
          </div>
        ...
        <div style={{ maxWidth: '900px', margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '2rem', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            <a href="https://www.paypal.com/us/fundraiser/charity/4550355" target="_blank" rel="noopener noreferrer" style={{
              ...
              borderRadius: '0.4rem', fontWeight: 700, fontSize: '0.9rem',
              letterSpacing: '0.04em', boxShadow: '0 0 20px rgba(255,0,106,0.3)',
            }}>
              Donate via PayPal
            </a>
